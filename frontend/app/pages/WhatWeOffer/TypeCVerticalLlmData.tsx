...
const VERTICALS = [
  { icon: "🚗", title: "Autonomous Driving", body: "2D, 3D & 4D LiDAR annotation, radar data labelling, camera fusion, and Driver Monitoring System datasets." },
  { icon: "🏥", title: "Healthcare & Medical", body: "Medical imaging annotation, clinical NLP datasets, diagnostic AI training data, and radiology report structuring." },
  { icon: "⚖", title: "Legal & Compliance", body: "Contract review datasets, legal entity extraction, regulatory document annotation, and jurisdiction-specific corpora." },
  { icon: "💰", title: "Finance & FinTech", body: "Transaction classification, fraud detection datasets, financial report extraction, and trading sentiment datasets." },
  { icon: "🏭", title: "Industrial & Manufacturing", body: "Defect detection training data, equipment monitoring datasets, and production line computer vision annotation." },
  { icon: "🚢", title: "Enterprise & Maritime", body: "Enterprise-grade private LLM datasets including ShipGPT-style maritime intelligence corpora for specialized domains." },
];
  ...
  return (
    <div style={{ borderBottom: "1px solid #e5e7eb" }}>
      <button onClick={onToggle} style={{ width: "100%", display: "flex", alignItems: "center", gap: 20, padding: "24px 0", background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
        <span style={{ fontSize: 11, fontWeight: 700, color: "#F5A623", background: "rgba(245,166,35,0.10)", padding: "4px 10px", borderRadius: 6, letterSpacing: "0.06em", flexShrink: 0 }}>{num}</span>
        <span style={{ fontSize: 17, fontWeight: 700, color: open ? "#2D6A4F" : "#1a1a1a", fontFamily: "Georgia, serif", flex: 1, transition: "color 0.2s" }}>{label}</span>
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ transform: open ? "rotate(180deg)" : "none", transition: "transform 0.25s", flexShrink: 0 }}>
          <path d="M4 6L8 10L12 6" stroke="#9ca3af" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </button>
      <div style={{ overflow: "hidden", maxHeight: open ? 220 : 0, transition: "max-height 0.35s ease" }}>
        <p style={{ fontSize: 14, color: "#6b7280", lineHeight: 1.75, paddingBottom: 24, margin: 0 }}>{content}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ background: "#fff", minHeight: "100vh" }}>
      <section style={{ maxWidth: 1400, margin: "0 auto", padding: "72px 80px 60px" }}>
        <TabIndicator />
        <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", color: "#1d4ed8", background: "#eff6ff", padding: "5px 12px", borderRadius: 999, display: "inline-block", marginBottom: 16 }}>Type C</span>
        <h1 style={{ fontSize: "clamp(2.4rem, 5vw, 4rem)", fontWeight: 800, color: "#1a1a1a", marginBottom: 24, lineHeight: 1.1, fontFamily: "Georgia, serif", letterSpacing: "-0.02em" }}>Vertical LLM Data</h1>
      ...
      <section style={{ maxWidth: 1400, margin: "0 auto", padding: "80px 80px 60px" }}>
        <div style={{ marginBottom: 48 }}>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.15em", color: "#F5A623", textTransform: "uppercase" as const, marginBottom: 12 }}>Industries</p>
          <h2 style={{ fontSize: "clamp(1.8rem, 3.5vw, 2.8rem)", fontWeight: 700, color: "#1a1a1a", fontFamily: "Georgia, serif", letterSpacing: "-0.02em", margin: 0 }}>Domain-specific AI data</h2>
        </div>
      ...
      <section style={{ maxWidth: 1400, margin: "0 auto", padding: "0 80px 100px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "start" }}>
          <div style={{ borderRadius: 20, background: "linear-gradient(135deg, #0f172a 0%, #1d4ed8 100%)", height: 420, display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: 40 }}>
            <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.15em", color: "#F5A623", textTransform: "uppercase" as const, marginBottom: 16 }}>Case Study</p>
            <h3 style={{ fontSize: 22, fontWeight: 700, color: "#fff", fontFamily: "Georgia, serif", lineHeight: 1.3, marginBottom: 20 }}>2D, 3D & 4D Data for Autonomous Driving</h3>
            <div style={{ display: "flex", gap: 24 }}>
              {[{ v: "99%", l: "Accuracy" }, { v: "250+", l: "Annotators" }].map(({ v, l }) => (
                <div key={l}><div style={{ fontSize: 24, fontWeight: 800, color: "#fff", fontFamily: "Georgia, serif" }}>{v}</div><div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", marginTop: 2 }}>{l}</div></div>
              ))}
            </div>
          </div>
          <div>
      ...
      <section style={{ background: "#111827", maxWidth: "calc(1400px - 160px)", margin: "0 auto 80px", borderRadius: 20, padding: "72px 80px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 40, flexWrap: "wrap" }}>
        <div style={{ maxWidth: 560 }}>
          <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.15em", color: "#F5A623", textTransform: "uppercase" as const, marginBottom: 16 }}>Get Started</p>
          <h2 style={{ fontSize: "clamp(1.8rem, 3vw, 2.6rem)", fontWeight: 700, color: "#fff", lineHeight: 1.2, fontFamily: "Georgia, serif", marginBottom: 16 }}>Need vertical AI training data?</h2>
          <p style={{ fontSize: 15, color: "#9ca3af", lineHeight: 1.75, margin: 0 }}>From autonomous vehicles to enterprise LLMs—we build the domain-specific datasets your models need.</p>
        </div>
